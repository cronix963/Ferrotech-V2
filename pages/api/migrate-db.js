...
export default async function handler(req, res) {
  ...
  try {
    ...
    const tables = [
      ...
      `CREATE TABLE IF NOT EXISTS ventas (id SERIAL PRIMARY KEY, pedido_id INTEGER, vendedor_id INTEGER, cliente_id INTEGER, total NUMERIC(12,2) NOT NULL DEFAULT 0, metodo_pago VARCHAR(50), estado VARCHAR(50) NOT NULL DEFAULT 'Completada', notas TEXT, created_at TIMESTAMP NOT NULL DEFAULT NOW())`,
      `CREATE TABLE IF NOT EXISTS compras (id SERIAL PRIMARY KEY, codigo VARCHAR(50) NOT NULL UNIQUE, proveedor_id INTEGER, proveedor VARCHAR(255) NOT NULL, items JSONB, total NUMERIC(12,2) NOT NULL DEFAULT 0, estado VARCHAR(50) NOT NULL DEFAULT 'Pendiente', notas TEXT, creado_por INTEGER, created_at TIMESTAMP NOT NULL DEFAULT NOW())`,
    ...
    // Users
    await query(`INSERT INTO users (email, password_hash, nombre, rol, provider, activo) VALUES
      ('admin@ferrotech.bo', '$2b$10$dE7kHORurC50Yjfh0NEoFuAH9fKvfn0.8GS/fQoyWxjob2oe.nGUW', 'Administrador', 'admin', 'credentials', TRUE),
      ('vendedor@ferrotech.bo', '$2b$10$pA3pJJi0Rdq/Fz0Rlx.H7.7x9NImO8wDBtmEM8r25u8FkDrwaISaG', 'Carlos Vendedor', 'vendedor', 'credentials', TRUE),
      ('cronix963@gmail.com', NULL, 'Miguel Fernandez', 'admin', 'google', TRUE)
    ON CONFLICT (email) DO NOTHING`);
    results.push('Usuarios insertados');

    // Clientes
    await query(`INSERT INTO clientes (nombre, email, telefono, direccion, ciudad, nit, empresa) VALUES
    ...
    // Proveedores
    await query(`INSERT INTO proveedores (nombre, contacto, email, telefono, direccion, ciudad) VALUES
    ...
    // Productos
    await query(`INSERT INTO productos (nombre, descripcion, categoria, precio, precio_compra, stock, icono, imagen) VALUES
    ...
    // Pedidos
    await query(`INSERT INTO pedidos (codigo, cliente_id, cliente, email, telefono, direccion, items, subtotal, impuesto, total, tipo, estado, pago, metodo_pago, notas, creado_por, created_at) VALUES
      ('#P-1718000001', 1, 'Constructora Los Andes', 'losandes@email.com', '7223456', 'Av. Ballivián 456, Santa Cruz', '[{"producto_id":1,"nombre":"Cemento Portland 50kg","cantidad":20,"precio":45.00},{"producto_id":2,"nombre":"Varilla 3/8 (12m)","cantidad":15,"precio":28.50},{"producto_id":3,"nombre":"Ladrillo Fiscal","cantidad":500,"precio":2.80}]'::jsonb, 2727.50, 0, 2727.50, 'tienda', 'Pendiente', 'Pendiente', 'Transferencia', 'Pedido grande para obra en construcción', 1, '2026-06-01 09:15:00'),
      ('#P-1718000002', 2, 'Ingeniería Santa Cruz', 'ing.sc@email.com', '7227890', 'Calle 21 de Mayo 789, Santa Cruz', '[{"producto_id":4,"nombre":"Martillo Stanley 16oz","cantidad":5,"precio":65.00},{"producto_id":5,"nombre":"Taladro Bosch 500W","cantidad":2,"precio":450.00},{"producto_id":11,"nombre":"Cinta Métrica 5m","cantidad":10,"precio":25.00}]'::jsonb, 1548.75, 73.75, 1548.75, 'tienda', 'En Proceso', 'Pendiente', 'Efectivo', 'Urgente — necesitan para proyecto en curso', 2, '2026-06-02 11:30:00'),
      ('#P-1718000003', 3, 'Distribuidora Norte', 'norte@email.com', '7224567', 'Av. Banzer 123, Santa Cruz', '[{"producto_id":6,"nombre":"Cable THW 2.5mm (100m)","cantidad":3,"precio":180.00},{"producto_id":12,"nombre":"Interruptor Simple","cantidad":20,"precio":8.50},{"producto_id":29,"nombre":"Foco LED 15W","cantidad":30,"precio":12.00}]'::jsonb, 1123.50, 53.50, 1123.50, 'pos', 'Completado', 'Pagado', 'Tarjeta', 'Entregado en mostrador', 2, '2026-05-28 16:45:00'),
      ('#P-1718000004', 1, 'Constructora Los Andes', 'losandes@email.com', '7223456', 'Av. Ballivián 456, Santa Cruz', '[{"producto_id":6,"nombre":"Cable THW 2.5mm (100m)","cantidad":5,"precio":180.00},{"producto_id":16,"nombre":"Cable THW 4mm (100m)","cantidad":3,"precio":290.00},{"producto_id":20,"nombre":"Disco de Corte 7","cantidad":10,"precio":22.00}]'::jsonb, 2089.50, 99.50, 2089.50, 'tienda', 'Despachado', 'Pagado', 'Transferencia', 'Enviado por transportista', 1, '2026-05-25 10:00:00'),
      ('#P-1718000005', 2, 'Ingeniería Santa Cruz', 'ing.sc@email.com', '7227890', 'Calle 21 de Mayo 789, Santa Cruz', '[{"producto_id":7,"nombre":"Tubo PVC 2 (6m)","cantidad":10,"precio":32.00},{"producto_id":19,"nombre":"Silicona Transparente","cantidad":5,"precio":15.00},{"producto_id":34,"nombre":"Cemento CPN 50kg","cantidad":10,"precio":42.00}]'::jsonb, 855.75, 40.75, 855.75, 'pos', 'Cancelado', 'Pendiente', 'Efectivo', 'Cliente canceló — pidió presupuesto nuevo', 2, '2026-05-20 14:20:00'),
      ('#P-1718000006', 3, 'Distribuidora Norte', 'norte@email.com', '7224567', 'Av. Banzer 123, Santa Cruz', '[{"producto_id":8,"nombre":"Pintura Látex 20L","cantidad":4,"precio":220.00},{"producto_id":14,"nombre":"Impermeabilizante 20L","cantidad":2,"precio":185.00},{"producto_id":38,"nombre":"Brocha 4 Profesional","cantidad":8,"precio":18.00}]'::jsonb, 1463.70, 69.70, 1463.70, 'tienda', 'Preparación', 'Pendiente', 'Tarjeta', 'Preparando para despacho', 2, '2026-06-05 08:00:00'),
      ('#P-1718000007', 1, 'Constructora Los Andes', 'losandes@email.com', '7223456', 'Av. Ballivián 456, Santa Cruz', '[{"producto_id":7,"nombre":"Tubo PVC 2 (6m)","cantidad":15,"precio":32.00},{"producto_id":25,"nombre":"Bomba de Agua 1HP","cantidad":1,"precio":520.00},{"producto_id":33,"nombre":"Grifo Cocina","cantidad":3,"precio":95.00}]'::jsonb, 1349.25, 64.25, 1349.25, 'tienda', 'Completado', 'Pagado', 'Transferencia', 'Entregado completo', 1, '2026-05-15 12:00:00'),
      ('#P-1718000008', 2, 'Ingeniería Santa Cruz', 'ing.sc@email.com', '7227890', 'Calle 21 de Mayo 789, Santa Cruz', '[{"producto_id":5,"nombre":"Taladro Bosch 500W","cantidad":1,"precio":450.00},{"producto_id":17,"nombre":"Llave Inglesa 12","cantidad":4,"precio":55.00},{"producto_id":20,"nombre":"Disco de Corte 7","cantidad":10,"precio":22.00},{"producto_id":24,"nombre":"Nivel de Burbuja 60cm","cantidad":3,"precio":35.00}]'::jsonb, 1044.75, 49.75, 1044.75, 'tienda', 'En Proceso', 'Pagado', 'Efectivo', 'Pagaron, estamos preparando el pedido', 2, '2026-06-08 09:30:00'),
      ('#P-1718000009', 3, 'Distribuidora Norte', 'norte@email.com', '7224567', 'Av. Banzer 123, Santa Cruz', '[{"producto_id":30,"nombre":"Sierra Circular Makita","cantidad":1,"precio":650.00},{"producto_id":31,"nombre":"Lijadora Orbital Bosch","cantidad":1,"precio":380.00}]'::jsonb, 1081.50, 51.50, 1081.50, 'tienda', 'Pendiente', 'Pendiente', 'Transferencia', 'Esperando confirmación de pago', 1, '2026-06-10 07:15:00'),
      ('#P-1718000010', 1, 'Constructora Los Andes', 'losandes@email.com', '7223456', 'Av. Ballivián 456, Santa Cruz', '[{"producto_id":10,"nombre":"Clavo 2 (1kg)","cantidad":5,"precio":12.00},{"producto_id":35,"nombre":"Tornillo Madera 3 (kg)","cantidad":3,"precio":22.00},{"producto_id":28,"nombre":"Cerradura Seguridad","cantidad":2,"precio":85.00}]'::jsonb, 310.80, 14.80, 310.80, 'pos', 'Completado', 'Pagado', 'Tarjeta', 'Compra en mostrador', 2, '2026-06-09 17:50:00')
    ON CONFLICT (codigo) DO NOTHING`);
    results.push('Pedidos insertados');
...
('#CO-1718000005', 2, 'Herramientas Pro', '[{"producto":"Taladro Bosch 500W","cantidad":5,"unidad":"pz","precio":350.00},{"producto":"Disco de Corte 7 pulg","cantidad":50,"unidad":"pz","precio":16.00}]'::jsonb, 2550.00, 'Recibido', 'Equipos eléctricos', '2026-06-04 10:00:00'),
...
((SELECT id FROM pedidos WHERE codigo = '#P-1718000001'), 2, 1, 2727.50, 'Transferencia', 'Venta completada - materiales de construcción', 'Completada', '2026-06-03 10:30:00'),
((SELECT id FROM pedidos WHERE codigo = '#P-1718000003'), 2, 3, 1123.50, 'Efectivo', 'Venta en mostrador - herramientas menores', 'Completada', '2026-06-04 15:00:00'),
((SELECT id FROM pedidos WHERE codigo = '#P-1718000004'), 1, 1, 2089.50, 'Efectivo', 'Venta de materiales para obra', 'Completada', '2026-06-05 09:15:00'),
((SELECT id FROM pedidos WHERE codigo = '#P-1718000007'), 2, 1, 1349.25, 'Tarjeta', 'Pago con tarjeta de crédito', 'Completada', '2026-06-07 11:45:00'),
((SELECT id FROM pedidos WHERE codigo = '#P-1718000009'), 1, 3, 1081.50, 'Transferencia', 'Transferencia bancaria confirmada', 'Completada', '2026-06-09 14:20:00'),
((SELECT id FROM pedidos WHERE codigo = '#P-1718000010'), 2, 1, 310.80, 'Efectivo', 'Venta rápida en POS', 'Completada', '2026-06-10 16:00:00'),
((SELECT id FROM pedidos WHERE codigo = '#P-1718000002'), 1, 2, 1548.75, 'Depósito', 'Pendiente de confirmación de depósito', 'Pendiente', '2026-06-02 08:00:00'),
((SELECT id FROM pedidos WHERE codigo = '#P-1718000008'), 2, 2, 1044.75, 'QR', 'Pago vía QR pendiente', 'Pendiente', '2026-06-08 10:00:00'),
((SELECT id FROM pedidos WHERE codigo = '#P-1718000006'), 1, 3, 1463.70, 'Depósito', 'Esperando confirmación del banco', 'Pendiente', '2026-06-06 08:30:00')
ON CONFLICT DO NOTHING`);
...
('Cobro', (SELECT id FROM ventas WHERE pedido_id = (SELECT id FROM pedidos WHERE codigo = '#P-1718000001') AND total = 2727.50), 'venta', 2727.50, 'Transferencia', 'Cobro venta #1 - Constructora Los Andes', '2026-06-03 10:35:00'),
('Cobro', (SELECT id FROM ventas WHERE pedido_id = (SELECT id FROM pedidos WHERE codigo = '#P-1718000003') AND total = 1123.50), 'venta', 1123.50, 'Efectivo', 'Cobro venta #3 - Distribuidora Norte', '2026-06-04 15:05:00'),
('Cobro', (SELECT id FROM ventas WHERE pedido_id = (SELECT id FROM pedidos WHERE codigo = '#P-1718000004') AND total = 2089.50), 'venta', 2089.50, 'Efectivo', 'Cobro venta #4 - Constructora Los Andes', '2026-06-05 09:20:00'),
('Cobro', (SELECT id FROM ventas WHERE pedido_id = (SELECT id FROM pedidos WHERE codigo = '#P-1718000007') AND total = 1349.25), 'venta', 1349.25, 'Tarjeta', 'Cobro venta #7 - Constructora Los Andes', '2026-06-07 11:50:00'),
('Cobro', (SELECT id FROM ventas WHERE pedido_id = (SELECT id FROM pedidos WHERE codigo = '#P-1718000009') AND total = 1081.50), 'venta', 1081.50, 'Transferencia', 'Cobro venta #9 - Distribuidora Norte', '2026-06-09 14:25:00'),
('Cobro', (SELECT id FROM ventas WHERE pedido_id = (SELECT id FROM pedidos WHERE codigo = '#P-1718000010') AND total = 310.80), 'venta', 310.80, 'Efectivo', 'Cobro venta #10 - Constructora Los Andes', '2026-06-10 16:05:00'),
